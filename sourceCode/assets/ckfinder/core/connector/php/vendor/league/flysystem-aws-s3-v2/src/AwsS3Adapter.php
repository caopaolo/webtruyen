...
class AwsS3Adapter extends AbstractAdapter
{
    ...
    public function __construct(
        ...
        // Mang tham so do Amobi them vao de luu cache file o local
        $expire = gmdate("D, d M Y H:i:s T", strtotime("+10 year"));
        
        $amobi_config = [
            'CacheControl'  => 'max-age=86400', // Tham so Amobi them vao de luu cache anh tai client
            'Expires'       => $expire // Tham so Amobi them vao de luu cache anh tai client
        ];
        $this->options = array_merge($this->options, $options, $amobi_config);
